...
# ---------------- PRE-IMPORT C-LINKING FIX ----------------
# Force-load libstdc++ into global namespace before swisseph loads
# This resolves the `undefined symbol: _ZSt7nothrow` error on Streamlit Cloud
try:
...
# ---------------- STREAMLIT INIT ----------------
st.set_page_config(
    page_title="FNO Reversal Dashboard",
    layout="wide",
    page_icon="🔮"
)

# Secret Authentication Handling with fallback
USERS = st.secrets.get("users", {"admin": "8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918"})
...
# ---------------- SYMBOL UNIVERSE ----------------
SYMBOLS = [
    'NIFTY', 'BANKNIFTY', 'CNXFINANCE', 'CNXMIDCAP', 'NIFTYJR',
    'RELIANCE', 'TCS', 'INFY', 'HDFCBANK', 'ICICIBANK'
]

# ---------------- ASTRO CONFIG & CALCULATIONS ----------------
...
def scan_astro(date, step=5):
    if not HAS_SWISSEPH:
        return "Unavailable", "Swisseph C-library failed to load", []
...
if stock_info:
    ...
    try:
        ...
        # Render static HTML with JavaScript execution enabled
        st.html(final_html, unsafe_allow_javascript=True)
    except FileNotFoundError:
        st.error("`dashboard.html` not found in repository root!")
else:
    st.error("Failed to fetch market data. Verify symbol or TV connection.")
